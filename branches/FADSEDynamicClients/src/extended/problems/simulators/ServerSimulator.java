...
import extended.problems.simulators.network.server.MessageSender;
import extended.problems.simulators.network.server.Neighbor;
import extended.problems.simulators.network.server.Neighborhood;
...
import extended.problems.simulators.network.server.ResultsReceiver;
import extended.problems.simulators.network.server.status.Simulation;
...
//    private Map<Message, Solution> sentToSimulation;
    ...
    public ServerSimulator(Environment environment) throws ClassNotFoundException, IOException, ParserConfigurationException {
        ...
        neighbors = new LinkedList<Neighbor>(NeighborhoodSingletonWrapper.getInstance().getNeighbors());
...
//                        Logger.getLogger(ServerSimulator.class.getName()).log(Level.INFO, "Found an available client... " + n.toString());
                    ...
                    //we need to remove the current client from the list...
                } catch (Exception ex) {
...
//TODO - TEST if the objectives are set in this method or calling set bad values is already too late
        ...
        try {
            ...
            //neighbors = Neighborhood_1.getRefreshedNeighbors();
            neighbors = new LinkedList<Neighbor>(NeighborhoodSingletonWrapper.getInstance().getNeighbors());
            Logger.getLogger(ServerSimulator.class.getName()).log(Level.CONFIG, "Loaded " + neighbors.size() + " neighbors...");
        } catch (ParserConfigurationException ex) {
            Logger.getLogger(ServerSimulator.class.getName()).log(Level.SEVERE, null, ex);
