...
// </editor-fold>
    ...
    public void performSimulation(Individual individual) {
        ...
        int feasible = -1;
        try {
            feasible = persistence.Result.isFeasible(individual);
        } catch (Exception e) {
        ...
        if (feasible < 0) {//NOT in the database
            ...
            try {
                persistence.Result.insertResult(individual.getEnvironment().getInputDocument(),
                        individual, simulatorOutputParser.fileContents.toString());
            } catch (Exception e) {
                System.out.println("ERROR WHILE ACCESING THE DATABASE");
            }
        } else if (feasible == 0) {//individual is in the databse but it is not feasible
            individual.markAsInfeasibleAndSetBadValuesForObjectives("individual is in the databse but it is not feasible");
        } else {//individual in the databse and feasible
