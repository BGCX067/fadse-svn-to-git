...
import environment.document.InputDocument.InputMetaOptimizedAlgorithm;
import environment.document.InputDocument.InputMetric;
...
import jmetal.metaheuristics.metaoptimizations.IsmailAlgorithm;
import jmetal.metaheuristics.metaoptimizations.metaoptimized.MetaOptimizedAlgorithm;
import jmetal.metaheuristics.metaoptimizations.metrics.Metric;
import jmetal.metaheuristics.metaoptimizations.metrics.MetricsFactory;
 ...
 * @author Cristian
 */
public class IsmailAlgorithm_Settings extends Settings {
    public int populationSize_ = 100;
    public int maxEvaluations_ = 2500;
    public double minimumPercentage_ = 0.1;
    
    private final List<InputMetaOptimizedAlgorithm> inputMoas;
    private final List<InputMetric> inputMetrics;
    
    /**
     * Creates an IsmailAlgorithm_Settings object for the creation of an 
     * instance of IsmailAlgorithm. It receives the problem to solve and 
     * the parameters from the configuration file for the meta-optimized
     * algorithms and for the metrics to be used.
     * 
     ...
     * @throws JMException 
     */
    public IsmailAlgorithm_Settings(Problem problem, List<InputMetaOptimizedAlgorithm> inputMoas, List<InputMetric> inputMetrics) throws JMException {
        super(problem);
    ...
    public Algorithm configure() throws JMException {
        IsmailAlgorithm algorithm = new IsmailAlgorithm(problem_);
        ...
        for (InputMetaOptimizedAlgorithm inputMoa : inputMoas) {
            Settings settings = (new SettingsFactory()).getSettingsObject(
                    "MetaOptimized" + inputMoa.getName(), new Object[] { problem_ });
            
            Properties properties = new Properties();
            try {
                properties.load(new FileInputStream(inputMoa.getConfigPath()));
            } catch (IOException ex) {
                Logger.getLogger(IsmailAlgorithm_Settings.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private double[] configureInitialPercentages() throws JMException {
        ...
        Set<InputMetaOptimizedAlgorithm> notAssigned = new HashSet<>();
        ...
        for (InputMetaOptimizedAlgorithm inputMoa : notAssigned) {
     ...
     * algorithms. Those which do not have a minimum percentage specified
     * are assigned the default minimum percentage of IsmailAlgorithm. If the
     * sum of the percentages exceeds 1.0 the method throws an error.
     * 
